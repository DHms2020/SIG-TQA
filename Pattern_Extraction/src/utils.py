    ...
    for i in range(st, ed):
        ...
        if 'rule_label' in sql:
            try:
                ...
                # print("-------to_batch_seq :index {}".format(idxes[i]))
            except:
                # print("execpt -------to_batch_seq :index {}".format(idxes[i]))
                continue
            if is_valid(rule_label, col_table_dict=col_table_dict, sql=sql) is False:
                # print("no valid -------to_batch_seq :index {}".format(idxes[i]))
                continue
...
def epoch_acc(model, batch_size, sql_data, table_data, beam_size=3):
    ...
    # print("-----验证集数据长度: {}".format(len(sql_data)))
    while st < len(sql_data):
        # print("the beginer of st :{}".format(st))
        ed = st+batch_size if st+batch_size < len(perm) else len(perm)
        examples = to_batch_seq(sql_data, table_data, perm, st, ed,
                                                        is_train=False)
        for i,example in enumerate(examples):
            ...
            # print("ed-st :{} example index :{} total index :{}".format((ed-st),i,len(json_datas)))
        st = ed
...
def load_dataset(dataset_dir, use_small=False):
    ...
    TRAIN_PATH = os.path.join(dataset_dir, "result_train.json")
    DEV_PATH = os.path.join(dataset_dir, "result_dev.json")
...
def init_log_checkpoint_path(args):
    ...
    save_path = os.path.join('./output', 'saved_model', dir_name)
