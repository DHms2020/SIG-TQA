...
class DataTrainingArguments:
    ...
    generate_data_file: Optional[str] = field(
        default=None,
        metadata={"help": "An optional input generate data file to produce query given sql"},
    )
...
def main():
    ...
    match_sequence_scorer = TextGenerationScorer(bos_id=label_bos_id, eos_id=label_eos_id, tokenizer=tokenizer,
                                                 output_path=os.path.join(training_args.output_dir, "eval_dump.json"))
    ...
    if training_args.do_train:
        ...
        logger.info("------------ready to train the model----------------")
    ...
    if training_args.do_generate:
        logger.info("*****Generate the query*******")
        model_path = (
            model_args.model_name_or_path
            if model_args.model_name_or_path is not None and os.path.isdir(model_args.model_name_or_path)
            else None
        ...
        # print(model_path)
        trainer.generate_run(model_path=model_path, generate_data_path=data_args.generate_data_file,
                             tokenizer=tokenizer, output_dir=training_args.output_dir, bos_id=label_bos_id,
                             eos_id=label_eos_id)
