...
from relogic.pretrainkit.my_transformers.models.bart import BartForConditionalGeneration
...
class SQL2TextModel(nn.Module):
  ...
  def __init__(self):
    super().__init__()
    self.bert = BartForConditionalGeneration.from_pretrained("relogic/facebook_bart_large")
  ...
  def forward(self, *input, **kwargs):
    ...
    if self.training:
      ...
      generated_ids, beam_scores = self.bert.generate(
        input_ids=input_ids,
      ...
      outputs = self.bert(input_ids,
                          attention_mask=attention_mask, decoder_input_ids=y_ids, labels=lm_labels, )
      return (outputs[0].detach(), (generated_ids,beam_scores))
