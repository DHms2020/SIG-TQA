...
class Trainer:
        ...
        optimizer_grouped_parameters = [
            {
                "params": [p for n, p in self.model.named_parameters() if
                           "relational_transformer" not in n and not any(nd in n for nd in no_decay)],
            ...
            {
                "params": [p for n, p in self.model.named_parameters() if
                           "relational_transformer" in n and not any(nd in n for nd in no_decay)],
                "weight_decay": self.args.weight_decay,
                "lr": 7e-5
            }
        ]
    ...
    def train(self, model_path: Optional[str] = None):
        ...
        train_iterator = trange(
            ...
            disable=not self.is_local_master() or not self.args.logging_tqdm
        )
    ...
    def _training_step(
        ...
        # print("----{} :ready to forward caculate----".format(self.global_step))
        outputs = model(**inputs)
        loss = outputs[0]  # model outputs are always tuple in transformers (see doc)
        # print("----{} :have the gradient----".format(self.global_step))
        if self.args.n_gpu > 1:
        ...
        if self.args.fp16:
            ...
            loss.backward()
        # print("-----{} :update the gradient----".format(self.global_step))
        return loss.item()
    ...
    def evaluate(
        ...
        # 保存最优模型
        if output.metrics["eval_bleu"] > self.best_bleu:
            self.best_bleu = output.metrics["eval_bleu"]
            output_dir = os.path.join(self.args.output_dir, "BestModel")
            self.save_model(output_dir)
    ...
    def generate_run(self, bos_id, eos_id, model_path: Optional[str] = None, generate_data_path: Optional[str] = None,
                     tokenizer: Optional[PreTrainedTokenizer] = None, output_dir: Optional[str] = None,
                     ):
        model = self.model
        ...
        if not os.path.exists(model_path):
            ...
            print("******model does eixst,now ready to load*****")

        model_param = torch.load(model_path)
        model.load_state_dict(model_param)
        logger.info("succed loade model from checkpoint")

        gen_dataset = SQL2QueryDataset(tokenizer=tokenizer, file_path=generate_data_path)
        gen_dataloader = self.get_eval_dataloader(eval_dataset=gen_dataset)
        if self.args.n_gpu > 1:
            model = torch.nn.DataParallel(model)
        batch_size = gen_dataloader.batch_size
        logger.info("***** Running Generator *****", )
        logger.info("  Num examples = %d", self.num_examples(gen_dataloader))
        ...
        beam_score_all: torch.Tensor = None
        model.eval()

        for inputs in tqdm(gen_dataloader, desc="Generator"):
            ...
            with torch.no_grad():
                outputs = model(**inputs)
                if has_labels:
                    step_eval_loss, logits_tuple_score = outputs[:2]
                    logits, beam_scores = logits_tuple_score
            ...
            if preds is None:
                ...
                preds_size = torch.cat(
                    (preds_size, logits.new_full(size=logits.size()[:1], fill_value=logits.size(1)).detach()),
                    dim=0)
            ...
            if inputs.get("labels") is not None:
                if label_ids is None:
                    ...
                    label_size = label_ids.new_full(size=label_ids.size()[:1],
                                                    fill_value=label_ids.size(1)).detach()
                    label_ids = label_ids.view(-1)
                else:
                    label_size = torch.cat((label_size, inputs["labels"].new_full(size=inputs["labels"].size()[:1],
                                                                                  fill_value=inputs["labels"].size(
                                                                                      1)).detach()), dim=0)
                    label_ids = torch.cat((label_ids, inputs["labels"].detach().view(-1)), dim=0)

            # 用beamscore作为置信度
            if beam_score_all is None:
                beam_score_all = beam_scores.detach()
            else:
                beam_score_all = torch.cat((beam_score_all, beam_scores.detach()), dim=0)
        ...
        #         EvalPredictionWithSize(predictions=preds, predictions_size=preds_size, label_ids=label_ids,
        ...
        p_start, l_start = 0, 0
        output_path = os.path.join(output_dir, "Generator.json")
        with open(output_path, "w") as fout:
            for idx, (p_size, l_size) in enumerate(zip(preds_size, label_size)):
                p_end = p_start + p_size
                l_end = l_start + l_size
                pred = get_sequence(preds[p_start: p_end])
                label = get_sequence(label_ids[l_start: l_end])
                p_start = p_end
                l_start = l_end
                ...
                fout.write(
                    json.dumps({
                        ...
                        "pred": pred_text,
                        "label": label_text,
                        "beam_score":beam_score_all[idx].item()}, ensure_ascii=False) + "\n")
    ...
    def _prediction_loop(
            ...
            if not prediction_loss_only:
                ...
                if inputs.get("labels") is not None:
                    if label_ids is None:
                        ...
                        label_size = torch.cat(
                            (label_size, inputs["labels"].new_full(size=inputs["labels"].size()[:1],
                                                                   fill_value=inputs["labels"].size(
                                                                       1)).detach()), dim=0)
                        label_ids = torch.cat((label_ids, inputs["labels"].detach().view(-1)), dim=0)
                #用beamscore作为置信度
        ...
        if self.args.local_rank != -1:
            ...
            if preds is not None:
                ...
                preds, preds_size = self.distributed_concat_with_size(preds, preds_size,
                                                                      num_total_examples=self.num_examples(
                                                                          dataloader))
            if label_ids is not None:
                # label_ids = self.distributed_concat(label_ids, num_total_examples=self.num_examples(dataloader))
                label_ids, label_size = self.distributed_concat_with_size(label_ids, label_size,
                                                                          num_total_examples=self.num_examples(
                                                                              dataloader))
        ...
        if self.compute_metrics is not None and preds is not None and label_ids is not None:
            ...
            metrics = self.compute_metrics(
                EvalPredictionWithSize(predictions=preds, predictions_size=preds_size, label_ids=label_ids,
                                       label_size=label_size), beam_score_all)
        else:
            metrics = {}
        ...
        return PredictionOutputWithSize(predictions=preds, predictions_size=preds_size, label_ids=label_ids,
                                        label_size=label_size, metrics=metrics)
    ...
    def distributed_concat_with_size(self, tensor: torch.Tensor, size: torch.Tensor,
