...
class EncDecModel(torch.nn.Module):
    class Preproc(abstract_preproc.AbstractPreproc):
        ...
        def validate_item_chinese(self, item, section):
            enc_result, enc_info = self.enc_preproc.validate_item_chinese(item, section)
            dec_result, dec_info = self.dec_preproc.validate_item(item, section)
        ...
        def add_item_chinese(self, item, section, validation_info):
            enc_info, dec_info = validation_info
            self.enc_preproc.add_item_chinese(item, section, enc_info)
            self.dec_preproc.add_item(item, section, dec_info)
    ...
    def _compute_loss_enc_batched(self, batch, debug=False):
        ...
        #调试用
        for record_id, (enc_state, (enc_input, dec_output)) in enumerate(zip(enc_states, batch)):
            print("in batch, this is the {} record".format(record_id))
