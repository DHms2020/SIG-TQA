...
class NL2CodeDecoderPreproc(abstract_preproc.AbstractPreproc):
    ...
    def map_TIME_NOW_to_id(self,cond):
        if cond['_type']=='Not':
            self.map_TIME_NOW_to_id(cond['c'])
        else:
            if 'col_unit2' in cond['val_unit']:
                if cond['val_unit']['col_unit1']['col_id']=='TIME_NOW':
                    cond['val_unit']['col_unit1']['col_id']=cond['val_unit']['col_unit2']['col_id']

    def validate_item(self, item, section):
        parsed = self.grammar.parse(item.code, section)
        if parsed:
            #对于DuSQL数据集中的TIME_NOW这种格式做特殊处理,转变为同级别col_id
            if 'sql_where' in parsed and 'where' in parsed['sql_where']:
                if 'left' in parsed['sql_where']['where']:
                    self.map_TIME_NOW_to_id(parsed['sql_where']['where']['left'])
                    self.map_TIME_NOW_to_id(parsed['sql_where']['where']['right'])
                else:
                    self.map_TIME_NOW_to_id(parsed['sql_where']['where'])
            #上述操作分割线
