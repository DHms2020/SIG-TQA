...
class SpiderEncoderV2Preproc(abstract_preproc.AbstractPreproc):
    ...
    def dataset(self, section):
        return [
            ...
            # for line in open(os.path.join(self.data_dir, 'bugtest.jsonl'))]
...
class Bertokens:
    ...
    def normalize_toks(self):
        ...
        for i, tok in enumerate(new_toks):
            ...
            lemmas = [tok.lemma.lower() for sent in doc.sentences for tok in sent.words]
...
class SpiderEncoderBartPreproc(SpiderEncoderV2Preproc):
    ...
    def __init__(
        ...
        column_types = ["text", "number", "time", "boolean", "others"]
        #pattern special token
        KEYWORDS = ['and', 'or', 'between', 'count', 'intersect', 'union', 'except']
        OP_TYPE=["CMP_OP","AGG_OP","CONDI_1_OP","CONDI_2_OP","ORDER_OP"]+KEYWORDS
        self.tokenizer.add_tokens([f"<type: {t}>" for t in column_types])
        self.tokenizer.add_tokens([t for t in column_types])
        self.tokenizer.add_tokens([t for t in OP_TYPE])
    ...
    def _tokenize_chinese(self, presplit, unsplit):
        #for the chinese dataset
        tokens = nltk.word_tokenize(unsplit.replace("'", " ' ").replace('"', ' " '))
        toks = []
        for token in tokens:
            toks.extend(self.berttokenizer.tokenize(token))
        return toks
    ...
    def add_item_chinese(self, item, section, validation_info):
        preprocessed = self.preprocess_item_chinese(item, validation_info)
        self.texts[section].append(preprocessed)
    ...
    def preprocess_item(self, item, validation_info):
        ...
        #Spider数据集转化为pattern后的训练集
        # question = self._tokenize(item.text, item.orig['pattern'])
        # value_list=item.orig['cvalues']
        # question = question + [self.tokenizer.sep_token] + value_list
        #迁移至DuSQL上的预处理
        question = self._tokenize(item.text, item.orig['query'])
        preproc_schema = self._preprocess_schema(item.schema)
        # question_bart_tokens = BartTokens(item.orig['pattern'], self.tokenizer)
        question_bart_tokens = BartTokens(item.orig['query'], self.tokenizer)
        if self.compute_sc_link:
        ...
        return {
            # 'raw_question': item.orig['pattern'],
            'raw_question': item.orig['query'],
            'question': question,
            'db_id': item.schema.db_id,
            'sc_link': sc_link,
            'cv_link': cv_link,
    ...
    def preprocess_item_chinese(self, item, validation_info):
        # For bart, there is a punctuation issue if we want to merge it back to words.
        # So here I will use nltk to further tokenize the sentence first.
        question = self._tokenize(item.text, item.orig['pattern'])
        value_list=item.orig['cvalues']
        question = question + [self.tokenizer.sep_token] + value_list
        preproc_schema = self._preprocess_schema_chinese(item.schema)
        question_bart_tokens = BartTokens(item.orig['pattern'], self.tokenizer)
        if self.compute_sc_link:
            # We do not want to transform pieces back to word.
            sc_link = question_bart_tokens.bart_schema_linking(
    ...
    def validate_item_chinese(self, item, section):
        question = self._tokenize(item.text, item.orig['pattern'])
        preproc_schema = self._preprocess_schema_chinese(item.schema)
        # 2 is for cls and sep special tokens. +1 is for sep
        num_words = len(question) + 2 + \
    ...
    def _preprocess_schema_chinese(self, schema):
        if schema.db_id in self.preprocessed_schemas:
            return self.preprocessed_schemas[schema.db_id]
        result = preprocess_schema_uncached_bart(schema, self.tokenizer, self._tokenize_chinese,
                                            self.include_table_name_in_column,
                                            self.fix_issue_16_primary_keys, bart=True)
        self.preprocessed_schemas[schema.db_id] = result
        return result
