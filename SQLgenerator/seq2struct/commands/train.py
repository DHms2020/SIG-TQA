...
class Trainer:
    def __init__(self, logger, config, local_rank):
        if torch.cuda.is_available():
            if local_rank != -1:
                torch.distributed.init_process_group(backend="nccl")
                self.num_worker = torch.distributed.get_world_size()
    ...
    def train(self, config, modeldir):
        ...
        with self.data_random:
            ...
            for batch_id, batch in enumerate(train_data_loader):
                ...
                # Evaluate model
                if self.local_rank in [-1, 0] and last_step % self.train_config.eval_every_n == 0:
                    print("----this is the {} batch".format(batch_id))
                    if self.train_config.eval_on_train:
                        self._eval_model(self.logger, self.model, last_step, train_eval_data_loader, 'train', num_eval_items=self.train_config.num_eval_items,local_rank=self.local_rank)
                    if self.train_config.eval_on_val:
                        self._eval_model(self.logger, self.model, last_step, val_data_loader, 'val', num_eval_items=self.train_config.num_eval_items,local_rank=self.local_rank)
    ...
    @staticmethod
    def _eval_model(logger, model, last_step, eval_data_loader, eval_section, num_eval_items=None,local_rank=-1):
        stats = collections.defaultdict(float)
        model.eval()
        with torch.no_grad():
          for eval_batch in eval_data_loader:
              if local_rank!=-1:
                batch_res=model.module.eval_on_batch(eval_batch)
              else:
...
def main(args):
    ...
    trainer = Trainer(logger, config, args.local_rank)
